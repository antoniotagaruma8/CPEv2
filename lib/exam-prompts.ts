...
- Gap-Fills: For gap-fill tasks, number the gaps sequentially within the text, e.g., "(1) _______". In the 'questions' array, set the 'question' field simply to "Gap 1", etc. CRITICAL DISTINCTION: If the part is a "Multiple-choice cloze", each question MUST have an 'options' array with EXACTLY 4 choices. If the part is an "Open cloze" or "Word formation", 'options' MUST be an empty array [] and you MUST provide a 'correctOption'.
...
IF the part is explicitly a "cloze" or "gap-fill" (e.g. "Multiple-choice cloze", "Open cloze", "Word formation"), you MUST physically remove the target words from the 'content' text and replace them INLINE with a numbered gap marker containing exactly 7 underscores.  
EXAMPLE REQUIRED INLINE FORMAT FOR GAPS: "The company announced a (1) _______ in their profits today."
CRITICALLY IMPORTANT: The gaps like "(1) _______", "(2) _______" MUST BE WRITTEN INSIDE THE PASSAGE TEXT! Never provide an intact text. Never just use "(1)" or "Gap 1" without the underscores!
...
- Part 2(Choice of task: ${taskTypesPart2}): Provide 'title' as "Part 2: Choose One Task", 'instructions'(MUST say: "Choose ONE of the tasks below and write your response in ${wordCountPart2}."). The 'content' field should be a brief thematic introduction to the tasks.
CRITICAL AUTHENTIC CAMBRIDGE FORMAT: The 'questions' array MUST contain EXACTLY 3 separate objects — each representing a DIFFERENT story-based task choice. Each object MUST have: 'question'(a unique task prompt describing a different story scenario, clearly labeled as "Task A:", "Task B:", or "Task C:"), empty 'options' array, 'imagePrompts' array containing EXACTLY 3 distinct highly descriptive prompts for generating 3 sequential images that tell THAT specific story, 'howToApproach'(a detailed 4-step guide), 'modelAnswer'(MUST strictly meet the ${wordCountPart2} requirement and showcase ${cefrLevel} complexity), and 'tips'(multi-sentence strategy). Each of the 3 tasks MUST be a distinctly different story scenario related to the overall topic.
${cambridgeStandards}
    `;
      } else {
        enhancedTopic = `Based on the ${topicPrompt}, generate a complete CEFR ${cefrLevel} Mock Writing exam.
...
- Part 2(Choice of task from: ${taskTypesPart2}): Provide 'title' as "Part 2: Choose One Task", 'instructions'(MUST say: "Choose ONE of the tasks below and write your response in ${wordCountPart2}."). The 'content' field should be a brief thematic introduction.
CRITICAL AUTHENTIC CAMBRIDGE FORMAT: The 'questions' array MUST contain EXACTLY 3 separate objects — each representing a DIFFERENT writing task type. Each object MUST have: 'question'(a unique, detailed task prompt clearly labeled as "Task A: [TASK TYPE]\\n\\n" followed by a scenario and clear instructions — e.g., "Task A: ARTICLE\\n\\nYou have seen the following announcement in an international magazine..."), empty 'options' array, 'howToApproach'(a detailed 4-step guide specific to that task type), 'modelAnswer'(MUST strictly meet the ${wordCountPart2} requirement and showcase high-level ${cefrLevel} complexity with explicitly separated \\n\\n paragraphs and proper formatting for that task type — e.g., letter format with greetings, article with headline, etc.), and 'tips'(multi-sentence strategy specific to that task type).
The 3 task types MUST be different from each other (e.g., one article, one letter/email, one report or review). Each must present a unique scenario related to the overall topic but requiring a different register, format, and approach.
${cambridgeStandards}
    `;
      }
      break;
